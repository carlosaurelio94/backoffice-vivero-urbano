...
const itemSchema = z.object({
  ...
  unit_price: z.coerce.number().min(0, 'Debe ser ≥ 0'),
...
// ─── Fila de ítem vacía por defecto ──────────────────────────────────────────
...
export default function NewQuotePage() {
  ...
  // FieldArray para los ítems dinámicos
  const { fields, append, remove } = useFieldArray({ control, name: 'items' });

  // Auto-rellenar el número de presupuesto cuando carga
  useEffect(() => {
    if (nextNumber) setValue('quote_number', nextNumber);
  }, [nextNumber, setValue]);

  // Observar ítems para calcular totales en tiempo real
  ...
  // Actualizar total_price de cada ítem cuando cambia qty o price
    ...
    <div className="flex flex-col gap-6 max-w-4xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">Completá los datos y agregá los ítems</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            {/* Preset de información (opcional) */}
            {infoList && infoList.length > 0 && (
              <div className="sm:col-span-2">
                <Select
                  label="Texto informativo (opcional)"
                  {...register('information_id')}
                >
        ...
        <div className="rounded-xl border border-gray-200 bg-white shadow-sm">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wide text-gray-500 w-[40%]">Producto / Servicio</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-gray-500 w-[15%]">Cantidad</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-gray-500 w-[20%]">Precio unit.</th>
                  <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wide text-gray-500 w-[20%]">Total</th>
                ...
                {fields.map((field, index) => (
                  <tr key={field.id}>
                    <td className="px-4 py-3">
                      <input
                        placeholder="Ej: Planta ornamental 40cm"
                    ...
                    <td className="px-4 py-3">
                      <input
                        ...
                        min="1"
                        step="1"
                        className="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-right
                                   focus:border-green-500 focus:outline-none focus:ring-1 focus:ring-green-500"
          ...
          {errors.items?.root && (
            <p className="px-6 pb-4 text-xs text-red-600">{errors.items.root.message}</p>
          )}
        </div>
